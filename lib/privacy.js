 ...
 */
function buildEvent({ kind, payload, cfg, agent, repo, branch }) {
  const safe = stripUnsafe(payload);
  const full = cfg.privacy === 'full';
  const sendsRepo = cfg.sendRepo !== false;

  // What the card is called.
  //
  // Two sources, and they are not the same kind of thing. A `task` piped in on
  // stdin is somebody's own words — CI naming a deploy, an agent describing
  // itself — so it is content, and content only leaves in full mode. A branch
  // is repository metadata, the same class as `repo` beside it: already written
  // down in the checkout, nowhere near the transcript. So it travels in both
  // modes and answers to the same switch `repo` does, because somebody who has
  // said not to send the repo's name has not agreed to send its branches.
  //
  // Without it every card is named "<agent> run": no agent's hook payload
  // carries a task, so `safe.task` is empty on every hook-driven event and a
  // board of nine runs reads as nine identical rows.
  const named = (full ? safe.task || null : null) || (sendsRepo ? branch || null : null);

  return {
    kind,
    agent,
    actor: cfg.actor,
    repo: sendsRepo ? repo : null,
    ...
    // reason is a category derived locally.
